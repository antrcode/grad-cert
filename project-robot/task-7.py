...
class MagicContainer(Container):
    ...
    Instance variables:
        ...
        subcontainers (List[Container]): List of subcontainers (empty for magic container).
    """
    ...
    def total_weight(self) -> int:
        """ Total weight is always equal to empty_weight. """
...
# update dictionary to include multicontainers
containers_dict = {c.name: c for c in containers}
...
with open("magic_containers.csv", "r") as fileref:
    for line in fileref.readlines()[1:]: # skip header
        ...
        if line:
            ...
            base_container = containers_dict.get(base_name)
            
            if base_container:
                containers.append(MagicContainer(name, base_container))

# dictionary for item look up
...
def loot_list(container: Container):
